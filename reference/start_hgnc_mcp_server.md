...
- port:
  ...
  Integer. Port number to run the server on (default: 8080). Only used
  when transport is "http".

- host:

  Character. Host address to bind to (default: "0.0.0.0"). Only used
  when transport is "http".

- transport:

  Character. Transport mode: "http" for HTTP/SSE transport or "stdio"
  for standard input/output (default: "http"). Use "stdio" for desktop
...
- ...:
  ...
  Additional arguments passed to `pr$run()` (for HTTP transport)
