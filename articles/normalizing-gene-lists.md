...
Gene nomenclature consistency is critical for clinical genomics
workflows. Inconsistent gene symbols can lead to:

- **Data integration errors** - Failed joins between datasets using
  different naming conventions
- **Clinical misinterpretation** - Confusion between genes with similar
  names or outdated symbols
- **Compliance issues** - Failure to meet regulatory requirements for
  standardized nomenclature

This vignette demonstrates how to use `hgnc.mcp` to normalize gene lists
for clinical panels, ensuring they comply with HGNC nomenclature
standards.
...
5.  **Duplicates** - The same gene listed multiple times with different
    symbols
...
Let’s start with a typical breast cancer panel that has some common
issues:
...
3.  **warnings** - Issues encountered (invalid symbols, withdrawn genes,
    etc.)
...
For clinical reporting, you often need to include cross-references to
other databases:
...
Sometimes a symbol might match multiple genes. Here’s how to handle
that:
...
to check if your gene panel meets HGNC nomenclature standards:
...
- **“HGNC”** (default) - Strict HGNC policy: only approved symbols
  allowed
- **“lenient”** - Allows approved symbols and some well-documented
  aliases
...
If you’re processing multiple gene lists, build a symbol index once and
reuse it:
...
- Explore [Working with HGNC Gene
  Groups](https://armish.github.io/hgnc.mcp/articles/gene-groups.md) to
  build panels from families
- See the [Getting
