...
Claude Desktop supports stdio transport for efficient local
communication. Configuration file location: - macOS:
...
**Other MCP Clients**: For HTTP-based clients, start the server with
`transport='http'` and connect to `http://localhost:8080/mcp`.
...
The easiest way to deploy the server is using Docker. Pre-built images
are **publicly available** on GitHub Container Registry and support
...
Images are automatically built and published on every push to the main
